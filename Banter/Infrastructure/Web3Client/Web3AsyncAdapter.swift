...
struct Web3AsyncAdapter {
  ...
  func getTransactionCount(
    ...
    block: EthereumQuantityTag = .latest
  ) async throws -> EthereumQuantity {
    let result = await asyncWrapper { callback in
...
private extension Web3Response.Status {
...
private func throwingAsyncWrapper<T>(
...
private func getResult<T>(_ data: T?, _ error: Error?) -> Swift.Result<T, Error> {
  if let error {
    .failure(error)
  } else if let data {
    .success(data)
